...
public class DisambiguationResultTest {
    ...
    public void testMeaning() {
        ...
        assertEquals("a device incised to make an impression; used to secure a closing or to authenticate documents", meaning.getDefinition());
    }
    ...
    public void testSentenceVariants() {
        ...
        assertEquals("pulse_n_02 them through the sieve_n_01 , return_n_11 them down seal_n_02 out in water_n_01 for a stew-pan in this is well some cold_water_n_01 , and a puree_n_01 with all , and in a shallow lawsuit_n_01 must already salted body_of_water_n_01 or location_n_01 .",
          lastVariant.toString());

        final ResolvedTerm pulseFromLastVariant = lastVariant.getTerms().get(0);
        final ResolvedTerm pulseFromFirstVariant = firstVariant.getTerms().get(0);

        assertSame(pulseFromFirstVariant, pulseFromLastVariant);
        assertEquals(1.0, pulseFromLastVariant.getScore(), 0.01);

        final ResolvedTerm stampFromFirstVariant = firstVariant.getTerms().get(9);
        final ResolvedTerm stampFromMiddleVariant = middleVariant.getTerms().get(9);
        ...
        assertEquals("seal_n_02", stampFromLastVariant.getMeaning().getMeaning());
        assertEquals(0.33, stampFromLastVariant.getScore(), 0.01);
    }

    @Test
    public void testSentenceVariantsForOneMeaningSentence() {        
        final ResolvedTerm flour_n_01 = singleMeaningVariant.getTerms().get(singleMeaningVariant.getTerms().size() - 2);

        assertEquals("Send them into another one can make little feculina flour .", singleMeaningsSentence.toString());
        assertEquals(1, singleMeaningVariants.size());

        assertEquals("Send them into another one can make little feculina flour_n_01 .", singleMeaningVariant.toString());
    ...
    public void testVariants() { 
    	...
    	assertEquals(singleMeaningVariant, variants.get(0).getSentences().get(1));
    	assertEquals(singleMeaningVariant, variants.get(1).getSentences().get(1));
    	assertEquals(singleMeaningVariant, variants.get(2).getSentences().get(1));
